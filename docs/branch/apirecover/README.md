...
- CLI 清理 recovery 标记但不会伪造用户 `?`；真实 Ctrl+C 后手动输入 `?` 的继续对话路径仍然保留
...
CLI 看到可恢复 transport failure 时，可以清理内部 `_auto_recovery_probe_pending` 状态并提示用户，但不能把内部恢复探针伪装成普通用户消息。

如果用户确实按 Ctrl+C 中断卡住状态并手动输入 `?`，这仍然是正常的真实用户 turn，应当继续被发送。
...
- 看到 `transport_failure[recovery_pending]` + `visible_transport_auto_recovery` 时，只记录/清理内部 recovery 状态
- 没有真实用户输入时，不会自动 queue `?`
- 如果用户已经有自己的中断/指令，则尊重用户输入；真实 Ctrl+C 后手动输入的 `?` 仍按正常用户消息发送
...
- CLI 是否禁止内部 recovery 在没有真实用户输入时伪造 `?`
...
- `scripts/run_tests.sh tests/cli/test_cli_api_recovery_probe.py -q`: 通过（2 passed；apirecover 工作树没有自带 venv，验证时临时创建 `.venv -> /Users/rain/hermes-agent/venv`，跑完已移除）。
- `git diff --check -- cli.py tests/cli/test_cli_api_recovery_probe.py docs/branch/apirecover/README.md docs/branch/apirecover/ai-brief.md`: 通过。
- 未运行全量测试；本轮验证聚焦 recovery 不伪造 `?` user turn。
...
- 有明确的 timeout 选择层次
- 有明确的恢复状态
- 有明确的 CLI recovery 边界：内部恢复状态不能伪造普通用户 `?` turn
- 有明确的测试约束
